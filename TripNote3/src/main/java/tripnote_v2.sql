...
COMMENT ON COLUMN user.user_nickname IS '회원닉네임';

COMMENT ON COLUMN user.user_pw IS '회원비밀번호';

COMMENT ON COLUMN user.user_birth IS '회원생년월일';

COMMENT ON COLUMN user.user_adminyn IS '회원관리자여부(관리자:Y, 일반회원:N)';
...
COMMENT ON COLUMN comment.posting_no IS '게시글번호';
...
ALTER TABLE userfavorite
	ADD
		CONSTRAINT FK_user_TO_userfavorite
		FOREIGN KEY (
			user_no
		)
		REFERENCES user (
...
ALTER TABLE following
	ADD
		CONSTRAINT FK_user_TO_following
		FOREIGN KEY (
			user_no
		)
		REFERENCES user (
...
ALTER TABLE comment
	ADD
		CONSTRAINT FK_posting_TO_comment
		FOREIGN KEY (
			posting_no
		)
		REFERENCES posting (
