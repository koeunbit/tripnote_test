...
var bootstrap = (function(exports, $, Popper) {
	...
	var Util = function() {
		...
		var TransitionEndEvent = {
			...
			transition : 'transitionend' // shoutout AngusCroll
											// (https://goo.gl/pxwQGp)
		...
		var Util = {
			...
			getUID : function getUID(prefix) {
				do {
					...
					prefix += ~~(Math.random() * MAX_UID); // "~~" acts like a
															// faster
															// Math.floor() here
			...
			typeCheckConfig : function typeCheckConfig(componentName, config,
				...
				for ( var property in configTypes) {
					if (Object.prototype.hasOwnProperty.call(configTypes,
						...
						var valueType = value && Util.isElement(value) ? 'element'
								: toType(value);
	...
	/**
	 * --------------------------------------------------------------------------
	 * Bootstrap (v4.0.0-beta.2): alert.js Licensed under MIT
	 * (https://github.com/twbs/bootstrap/blob/master/LICENSE)
	 * --------------------------------------------------------------------------
	 */
	...
	var Carousel = function() {
		...
		var ARROW_LEFT_KEYCODE = 37; // KeyboardEvent.which value for left
										// arrow key

		var ARROW_RIGHT_KEYCODE = 39; // KeyboardEvent.which value for right
										// arrow key

		var TOUCHEVENT_COMPAT_WAIT = 500; // Time for mouse compat events to
											// fire after touch
		...
		function() {
			...
			_proto._addEventListeners = function _addEventListeners() {
				...
				if (this._config.pause === 'hover') {
					...
					if ('ontouchstart' in document.documentElement) {
						...
						// fired as
						// part of the mouse compatibility events on first tap -
						// the carousel
						// would stop cycling until user tapped out of it;
						// here, we listen for touchend, explicitly pause the
						// carousel
			...
			_proto._getItemIndex = function _getItemIndex(element) {
				this._items = $.makeArray($(element).parent().find(
						Selector.ITEM));
			...
			_proto._triggerSlideEvent = function _triggerSlideEvent(
				...
				var fromIndex = this._getItemIndex($(this._element).find(
						Selector.ACTIVE_ITEM)[0]);
			...
			_proto._setActiveIndicatorElement = function _setActiveIndicatorElement(
				...
				if (this._indicatorsElement) {
					...
					var nextIndicator = this._indicatorsElement.children[this
							...
							function() {
								$(nextElement).removeClass(
										directionalClassName + " "
												+ orderClassName).addClass(
										ClassName.ACTIVE);
								$(activeElement).removeClass(
										ClassName.ACTIVE + " " + orderClassName
												+ " " + directionalClassName);
	...
	var Collapse = function() {
		...
		function() {
			...
			_proto.show = function show() {
				...
				if (this._parent) {
					actives = $.makeArray($(this._parent).children().children(
							Selector.ACTIVES));
				...
				$(this._element).removeClass(ClassName.COLLAPSE).addClass(
				...
				var complete = function complete() {
					$(_this._element).removeClass(ClassName.COLLAPSING)
							.addClass(ClassName.COLLAPSE).addClass(
									ClassName.SHOW);
			...
			_proto.hide = function hide() {
				...
				$(this._element).addClass(ClassName.COLLAPSING).removeClass(
						ClassName.COLLAPSE).removeClass(ClassName.SHOW);
					...
					for (var i = 0; i < this._triggerArray.length; i++) {
						...
						if (selector !== null) {
							...
							if (!$elem.hasClass(ClassName.SHOW)) {
								$(trigger).addClass(ClassName.COLLAPSED).attr(
										'aria-expanded', false);
							}
						}
					}
				...
				var complete = function complete() {
					...
					$(_this2._element).removeClass(ClassName.COLLAPSING)
			...
			_proto._getParent = function _getParent() {
				...
				var selector = "[data-toggle=\"collapse\"][data-parent=\""
						+ this._config.parent + "\"]";
				$(parent).find(selector).each(
						function(i, element) {
							_this3._addAriaAndCollapsedClass(Collapse
		...
		$(document).on(Event.CLICK_DATA_API, Selector.DATA_TOGGLE,
				function(event) {
					// preventDefault only for <a> elements (which change the
					// URL) not inside the collapsible element
	...
	var Dropdown = function() {
		...
		var DATA_API_KEY = '.data-api';
		var JQUERY_NO_CONFLICT = $.fn[NAME];
		var ESCAPE_KEYCODE = 27; // KeyboardEvent.which value for Escape
									// (Esc) key
		...
		var ARROW_UP_KEYCODE = 38; // KeyboardEvent.which value for up arrow
									// key

		var ARROW_DOWN_KEYCODE = 40; // KeyboardEvent.which value for down
										// arrow key

		var RIGHT_MOUSE_BUTTON_WHICH = 3; // MouseEvent.which value for the
											// right button (assuming a
											// right-handed mouse)
		...
		function() {
			...
			_proto.toggle = function toggle() {
				...
				var element = this._element; // for dropup with alignment we
												// use the parent as popper
												// container
				...
				this._popper = new Popper(element, this._menu, this
						._getPopperConfig()); // if this is a touch-enabled
												// device we add extra
				...
				$(parent).toggleClass(ClassName.SHOW).trigger(
			...
			_proto._getPopperConfig = function _getPopperConfig() {
				...
				if (typeof this._config.offset === 'function') {
					offsetConf.fn = function(data) {
						data.offsets = $.extend({}, data.offsets,
			...
			Dropdown._clearMenus = function _clearMenus(event) {
				if (event
						&& (event.which === RIGHT_MOUSE_BUTTON_WHICH || event.type === 'keyup'
				...
				for (var i = 0; i < toggles.length; i++) {
					...
					if (event
							&& (event.type === 'click'
									&& /input|textarea/i
											.test(event.target.tagName) || event.type === 'keyup'
									&& event.which === TAB_KEYCODE)
					...
					$(parent).removeClass(ClassName.SHOW).trigger(
							$.Event(Event.HIDDEN, relatedTarget));
				}
			};
		...
		$(document).on(Event.KEYDOWN_DATA_API, Selector.DATA_TOGGLE,
				...
				Selector.MENU, Dropdown._dataApiKeydownHandler).on(
		...
		function() {
			...
			_proto._setScrollbar = function _setScrollbar() {
				...
				if (this._isBodyOverflowing) {
					// Note: DOMNode.style.paddingRight returns the actual value
					// or '' if not set
					...
					$('body').data('padding-right', actualPadding).css(
							'padding-right',
							parseFloat(calculatedPadding)
			...
			_proto._resetScrollbar = function _resetScrollbar() {
				...
				if (typeof padding !== 'undefined') {
					$('body').css('padding-right', padding).removeData(
							'padding-right');
				}
			};
			...
			Modal._jQueryInterface = function _jQueryInterface(config,
				...
				return this.each(function() {
					...
					var _config = $.extend({}, Modal.Default, $(this).data(),
							typeof config === 'object' && config);
		...
		$(document).on(
				...
				function(event) {
					...
					var $target = $(target).one(Event.SHOW,
							function(showEvent) {
								if (showEvent.isDefaultPrevented()) {
									// only register focus restorer if modal
		...
		function() {
			...
			_proto.show = function show() {
				...
				if (this.isWithContent() && this._isEnabled) {
					...
					var placement = typeof this.config.placement === 'function' ? this.config.placement
							.call(this, tip, this.element)
					...
					$(tip).addClass(ClassName.SHOW); // if this is a
														// touch-enabled device
														// we add extra
					// empty mouseover listeners to the body's immediate
					// children;
					// only needed because of broken event delegation on iOS
					// https://www.quirksmode.org/blog/archives/2014/02/mouse_event_bub.html
			...
			_proto.hide = function hide(callback) {
				...
				$(tip).removeClass(ClassName.SHOW); // if this is a
													// touch-enabled device we
													// remove the extra
				// empty mouseover listeners we added for iOS support
			...
			_proto.setContent = function setContent() {
				var $tip = $(this.getTipElement());
				this.setElementContent($tip.find(Selector.TOOLTIP_INNER), this
						.getTitle());
				$tip.removeClass(ClassName.FADE + " " + ClassName.SHOW);
			};

			_proto.setElementContent = function setElementContent($element,
					content) {
			...
			_proto._fixTitle = function _fixTitle() {
				var titleType = typeof this.element
			...
			_proto._enter = function _enter(event, context) {
				...
				if (event) {
					context._activeTrigger[event.type === 'focusin' ? Trigger.FOCUS
							: Trigger.HOVER] = true;
				}
			...
			_proto._leave = function _leave(event, context) {
				...
				if (event) {
					context._activeTrigger[event.type === 'focusout' ? Trigger.FOCUS
							: Trigger.HOVER] = false;
				}
			...
			_proto._getConfig = function _getConfig(config) {
				config = $.extend({}, this.constructor.Default, $(this.element)
						.data(), config);
	...
	var Popover = function() {
		...
		function(_Tooltip) {
			...
			_proto.setContent = function setContent() {
				var $tip = $(this.getTipElement()); // we use append for html
													// objects to maintain js
													// events

				this.setElementContent($tip.find(Selector.TITLE), this
						.getTitle());
				this.setElementContent($tip.find(Selector.CONTENT), this
						._getContent());
				$tip.removeClass(ClassName.FADE + " " + ClassName.SHOW);
			}; // private
			...
			_proto._getContent = function _getContent() {
				return this.element.getAttribute('data-content')
						|| (typeof this.config.content === 'function' ? this.config.content
								.call(this.element)
								: this.config.content);
			};
		...
		function() {
			...
			_proto.refresh = function refresh() {
				...
				var autoMethod = this._scrollElement !== this._scrollElement.window ? OffsetMethod.POSITION
						: OffsetMethod.OFFSET;
				var offsetMethod = this._config.method === 'auto' ? autoMethod
						: this._config.method;
				var offsetBase = offsetMethod === OffsetMethod.POSITION ? this
						._getScrollTop() : 0;
				this._offsets = [];
				this._targets = [];
						...
						function(element) {
							...
							if (target) {
								...
								if (targetBCR.width || targetBCR.height) {
									// todo (fat): remove sketch reliance on
			...
			_proto._activate = function _activate(target) {
				...
				if ($link.hasClass(ClassName.DROPDOWN_ITEM)) {
					$link.closest(Selector.DROPDOWN).find(
							Selector.DROPDOWN_TOGGLE)
							.addClass(ClassName.ACTIVE);
					$link.addClass(ClassName.ACTIVE);
				} else {
					// Set triggered link as active
					$link.addClass(ClassName.ACTIVE); // Set triggered links
														// parents as active
					// With both <ul> and <nav> markup a parent is the previous
					// sibling of any nav ancestor

					$link.parents(Selector.NAV_LIST_GROUP).prev(
							Selector.NAV_LINKS + ", " + Selector.LIST_ITEMS)
							.addClass(ClassName.ACTIVE); // Handle special
															// case when
															// .nav-link is
															// inside .nav-item

					$link.parents(Selector.NAV_LIST_GROUP).prev(
							Selector.NAV_ITEMS).children(Selector.NAV_LINKS)
							.addClass(ClassName.ACTIVE);
				}
			...
			_proto._clear = function _clear() {
				$(this._selector).filter(Selector.ACTIVE).removeClass(
						ClassName.ACTIVE);
			}; // static
		...
		function() {
			...
			_proto.show = function show() {
				...
				var listElement = $(this._element).closest(
						Selector.NAV_LIST_GROUP)[0];
				var selector = Util.getSelectorFromElement(this._element);

				if (listElement) {
					var itemSelector = listElement.nodeName === 'UL' ? Selector.ACTIVE_UL
							: Selector.ACTIVE;
			...
			_proto._transitionComplete = function _transitionComplete(element,
					active, isTransitioning, callback) {
				if (active) {
					$(active).removeClass(ClassName.ACTIVE);
					var dropdownChild = $(active.parentNode).find(
							Selector.DROPDOWN_ACTIVE_CHILD)[0];
	...
	(function() {
		...
		if (version[0] < ltMajor && version[1] < minMinor
				|| version[0] === minMajor && version[1] === minMinor
